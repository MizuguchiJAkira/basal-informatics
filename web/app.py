...
def create_app(demo: bool = False, site: str = "strecker") -> Flask:
    ...
    # /demo/run — JSON POST from the in-page Run Assessment button.
    # Same posture as the lender override: ``demo_bp`` mixes HTML
    # (the /demo page itself) and JSON (this endpoint), so we exempt
    # the function, not the blueprint.
    from web.routes.demo import run_assessment
    csrf.exempt(run_assessment)
    ...
    def active_site() -> str:
        ...
        # Explicit override via query string wins — lets you preview
        # either brand on any host (?site=strecker on basal.eco, etc).
